...
## Recent Changes
- **v2.9.1 (Fix App button)** — Added a `🔧 Fix App` button to the dashboard footer (next to `🔒 Lock App`). On click it shows a confirm dialog ("App ko refresh karein? Isse errors theek ho sakte hain."); if accepted it (1) clears all `caches` storage entries via `caches.keys() → caches.delete()`, (2) unregisters every Service Worker via `navigator.serviceWorker.getRegistrations() → unregister()`, then (3) calls `window.location.reload(true)` (with a plain `reload()` fallback) to force a fresh download from the server. Button uses the same pill style as `.dash-logout` but hovers to accent-orange. SW cache bumped to `pccs-awb-v2.9.1-fixapp`.
