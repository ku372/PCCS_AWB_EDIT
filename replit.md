...
## Recent Changes
- **v2.8.6 (TAX border patch v5.5 — merge with original border)** — v5.4 produced a visible **double-line** under the TAX value: my line landed ~2.5pt ABOVE the form's original bottom border. Per user's instruction, moved bottom line DOWN another 2.5pt (`taxWriteY-4.3 → taxWriteY-6.8`) so it sits ON the original border line, eliminating the double-line effect. Right vertical (`125.5`) and thickness (`0.35`) kept. SW cache bumped to `pccs-awb-v2.8.6-taxborder`.
- **v2.8.5 (TAX border patch v5.4)** — Bottom moved to `taxWriteY-4.3`, right to `125.5` — produced double-line.
