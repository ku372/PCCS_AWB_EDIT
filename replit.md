...
## Recent Changes
- **v2.8.1 (TAX border patch v5)** — White-rect inset alone (v4) wasn't enough to close the visible gaps on the TAX cell's right vertical and bottom horizontal. Added two thin black overlay lines via `page.drawLine()` after the TAX text writes: a vertical at `x=124` from `taxWriteY-1` to `dotsY0+5`, and a horizontal at `y=taxWriteY-1` from `x=58` to `x=124`. Lines avoid both the value (right-aligned at x≈107) and the TAX label (at x=138), so no text is touched. SW cache bumped to `pccs-awb-v2.8.1-taxborder`.
