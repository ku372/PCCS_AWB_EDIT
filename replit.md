...
## Recent Changes
- **v2.8.2 (TAX border patch v5.1 — alignment + thinner)** — Mobile preview showed v5 lines were ~1pt too low and slightly outside the existing right vertical, and looked too bold. Tweaked: `TAX_BOT_Y` raised from `taxWriteY-1.0` to `taxWriteY-0.3` (now hugs the value baseline; cannot go higher without crossing digits since they have no descender), `TAX_BOX_RIGHT` shifted inward `124 → 123.5`, and line `thickness: 0.5 → 0.35` to match the form's hairlines. SW cache bumped to `pccs-awb-v2.8.2-taxborder`.
